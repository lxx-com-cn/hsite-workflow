package com.hbs.site.framework.test.core.util;
...
public class AssertUtils {
     ...
     *
     * 注意：
     * 1. 如果 expected 存在的属性，actual 不存在的时候，会进行忽略
     * 2. 对于 LocalDateTime 类型字段，会自动截断到秒进行比较，忽略毫秒差异
     *
     * @param expected 期望对象
     * @param actual 实际对象
    ...
    public static void assertPojoEquals(Object expected, Object actual, String... ignoreFields) {
        ...
        Arrays.stream(expectedFields).forEach(expectedField -> {
            ...
            // 特殊处理：如果是 LocalDateTime 类型，截断到秒进行比较
            if (expectedValue instanceof LocalDateTime && actualValue instanceof LocalDateTime) {
                LocalDateTime truncatedExpected = ((LocalDateTime) expectedValue).truncatedTo(ChronoUnit.SECONDS);
                LocalDateTime truncatedActual = ((LocalDateTime) actualValue).truncatedTo(ChronoUnit.SECONDS);
    ...
    public static boolean isPojoEquals(Object expected, Object actual, String... ignoreFields) {
        ...
        return Arrays.stream(expectedFields).allMatch(expectedField -> {
            ...
            if (expectedValue instanceof LocalDateTime && actualValue instanceof LocalDateTime) {
                LocalDateTime truncatedExpected = ((LocalDateTime) expectedValue).truncatedTo(ChronoUnit.SECONDS);
                LocalDateTime truncatedActual = ((LocalDateTime) actualValue).truncatedTo(ChronoUnit.SECONDS);
                return Objects.equals(truncatedExpected, truncatedActual);
            } else {
