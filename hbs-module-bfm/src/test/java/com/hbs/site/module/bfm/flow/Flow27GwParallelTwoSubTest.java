...
public class Flow27GwParallelTwoSubTest {
    ...
    public void setUp() throws Exception {
        ...
        String xmlContent = loadXmlFromClasspath("flow/flow27v10-gw-parallel-2sub.xml");
        runtimePackage = orchestrationEngine.deployPackage(xmlContent);

        log.info("✅ 并行网关-双子流程包部署成功: packageId={}, workflows={}",
    ...
    private void validateResult(ProcessInstance processInstance, String seq) {
        ...
        // 验证分支标识（来自子流程内部）
        assertEquals("BUSINESS", result.get("postBranch"), "岗位分支标识应为BUSINESS");
        assertEquals("BUSINESS", result.get("userBranch"), "用户分支标识应为BUSINESS");
    ...
    public void testFullParallelFlow() throws Exception {
        ...
        ProcessInstance processInstance = orchestrationEngine.startProcess(
                ...
                "main-parallel-2sub",
                "1.0.0",
                "TEST-PARALLEL-2SUB-" + uniqueSuffix,
                variables
        );
    ...
    public void testSubPostProcessStandalone() throws Exception {
        ...
        String postBranch = (String) subResult.get("postBranch");

        assertNotNull(postId, "postId应为非空");
        assertNotNull(postDetail, "postDetail应为非空");
        assertEquals("岗位_" + uniqueSuffix + "_" + seq, postDetail.getName(), "岗位名称应匹配");
        assertEquals("BUSINESS", postBranch, "岗位分支标识应为BUSINESS");
    ...
    public void testSubUserProcessStandalone() throws Exception {
        ...
        String userBranch = (String) subResult.get("userBranch");

        assertNotNull(userId, "userId应为非空");
        assertNotNull(userDetail, "userDetail应为非空");
        assertEquals("user_" + uniqueSuffix + "_" + seq, userDetail.getUsername(), "用户名应匹配");
        assertEquals("BUSINESS", userBranch, "用户分支标识应为BUSINESS");
