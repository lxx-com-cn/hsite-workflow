...
public class Flow31GwInclusiveTwoSubTest {
    ...
    private static final List<String> FLAGS = Arrays.asList("A", "B", "C", "AB", "AC", "BC", "ABC");
    ...
    public void setUp() throws Exception {
        ...
        String xmlContent = loadXmlFromClasspath("flow/flow31v10-gw-inclusive-2sub.xml");
        runtimePackage = orchestrationEngine.deployPackage(xmlContent);

        log.info("✅ 包容网关-双子流程包部署成功: packageId={}, workflows={}",
    ...
    private UserSaveReqVO createUserReq(String suffix, int seqNum) {
        ...
        req.setSex(1);
        String mobileSuffix = String.format("%03d", seqNum); // 3位序号
    ...
    private void validateResult(ProcessInstance processInstance,
                                String mainFlags, String subFlags1, String subFlags2) {
        log.info("验证组合: main={}, sub1={}, sub2={}", mainFlags, subFlags1, subFlags2);
        ...
        for (char c : subFlags1.toCharArray()) {
            expectedSub1.add(String.valueOf(c));
        }
        Set<String> actualSub1 = new HashSet<>();
        if (result.get("sub1A") != null) actualSub1.add("A");
        if (result.get("sub1B") != null) actualSub1.add("B");
        if (result.get("sub1C") != null) actualSub1.add("C");
        ...
        for (char c : subFlags2.toCharArray()) {
            expectedSub2.add(String.valueOf(c));
        }
        Set<String> actualSub2 = new HashSet<>();
        if (result.get("sub2A") != null) actualSub2.add("A");
        if (result.get("sub2B") != null) actualSub2.add("B");
        if (result.get("sub2C") != null) actualSub2.add("C");
    ...
    public void testAllCombinations() throws Exception {
        log.info("\n========== 开始测试所有组合 ({} 种) ==========",
                FLAGS.size() * FLAGS.size() * FLAGS.size());

        int total = 0;
        for (String mainFlags : FLAGS) {
            for (String subFlags1 : FLAGS) {
                for (String subFlags2 : FLAGS) {
                    total++;
                    log.info("\n--- 测试组合 {}/343: main={}, sub1={}, sub2={} ---",
                            total, mainFlags, subFlags1, subFlags2);

                    Map<String, Object> variables = new HashMap<>();
                    variables.put("flags", mainFlags);
                    variables.put("subFlags1", subFlags1);
                    variables.put("subFlags2", subFlags2);

                    if (mainFlags.contains("A")) {
                        variables.put("deptRequest", createDeptReq(mainFlags + "_" + subFlags1 + "_" + subFlags2));
                    }
                    if (mainFlags.contains("B")) {
                        variables.put("postRequest", createPostReq(mainFlags + "_" + subFlags1 + "_" + subFlags2));
                    }
                    if (mainFlags.contains("C")) {
                        int seqNum = userCounter.incrementAndGet();
                        variables.put("userRequest", createUserReq(mainFlags + "_" + subFlags1 + "_" + subFlags2, seqNum));
                    }
                    ...
                    ProcessInstance processInstance = orchestrationEngine.startProcess(
                            ...
                            "inclusive-main",
                            "1.0.0",
                            String.format("TEST-%s-%s-%s-%s", mainFlags, subFlags1, subFlags2, uniqueSuffix),
                            variables
                    );
                    ...
                    validateResult(processInstance, mainFlags, subFlags1, subFlags2);
                }
            }
        }
    ...
    public void testMainDefaultBranch() throws Exception {
        ...
        for (String subFlags1 : FLAGS) {
            for (String subFlags2 : FLAGS) {
                log.info("sub1={}, sub2={}", subFlags1, subFlags2);
                ...
                ProcessInstance processInstance = orchestrationEngine.startProcess(
                        ...
                        "inclusive-main",
                        "1.0.0",
                        String.format("TEST-MAIN-DEFAULT-%s-%s-%s", subFlags1, subFlags2, uniqueSuffix),
                        variables
                );
                ...
                Set<String> expectedSub1 = new HashSet<>();
                for (char c : subFlags1.toCharArray()) expectedSub1.add(String.valueOf(c));
                Set<String> actualSub1 = new HashSet<>();
                if (result.get("sub1A") != null) actualSub1.add("A");
                if (result.get("sub1B") != null) actualSub1.add("B");
                if (result.get("sub1C") != null) actualSub1.add("C");
                ...
                Set<String> expectedSub2 = new HashSet<>();
                for (char c : subFlags2.toCharArray()) expectedSub2.add(String.valueOf(c));
                Set<String> actualSub2 = new HashSet<>();
                if (result.get("sub2A") != null) actualSub2.add("A");
                if (result.get("sub2B") != null) actualSub2.add("B");
                if (result.get("sub2C") != null) actualSub2.add("C");
    ...
    public void testSub1DefaultBranch() throws Exception {
        ...
        for (String mainFlags : FLAGS) {
            for (String subFlags2 : FLAGS) {
                log.info("main={}, sub2={}", mainFlags, subFlags2);
                ...
                if (mainFlags.contains("A")) {
                    variables.put("deptRequest", createDeptReq(mainFlags + "_DEFAULT_" + subFlags2));
                }
                if (mainFlags.contains("B")) {
                    variables.put("postRequest", createPostReq(mainFlags + "_DEFAULT_" + subFlags2));
                }
                if (mainFlags.contains("C")) {
                    int seqNum = userCounter.incrementAndGet();
                    variables.put("userRequest", createUserReq(mainFlags + "_DEFAULT_" + subFlags2, seqNum));
                }
                ...
                ProcessInstance processInstance = orchestrationEngine.startProcess(
                        ...
                        "inclusive-main",
                        "1.0.0",
                        String.format("TEST-SUB1-DEFAULT-%s-%s-%s", mainFlags, subFlags2, uniqueSuffix),
                        variables
                );
                ...
                assertNull(result.get("sub1A"));
                assertNull(result.get("sub1B"));
                assertNull(result.get("sub1C"));
    ...
    public void testSub2DefaultBranch() throws Exception {
        ...
        for (String mainFlags : FLAGS) {
            for (String subFlags1 : FLAGS) {
                log.info("main={}, sub1={}", mainFlags, subFlags1);
                ...
                if (mainFlags.contains("A")) {
                    variables.put("deptRequest", createDeptReq(mainFlags + "_" + subFlags1 + "_DEFAULT"));
                }
                if (mainFlags.contains("B")) {
                    variables.put("postRequest", createPostReq(mainFlags + "_" + subFlags1 + "_DEFAULT"));
                }
                if (mainFlags.contains("C")) {
                    int seqNum = userCounter.incrementAndGet();
                    variables.put("userRequest", createUserReq(mainFlags + "_" + subFlags1 + "_DEFAULT", seqNum));
                }
                ...
                ProcessInstance processInstance = orchestrationEngine.startProcess(
                        ...
                        "inclusive-main",
                        "1.0.0",
                        String.format("TEST-SUB2-DEFAULT-%s-%s-%s", mainFlags, subFlags1, uniqueSuffix),
                        variables
                );
                ...
                assertNull(result.get("sub2A"));
                assertNull(result.get("sub2B"));
                assertNull(result.get("sub2C"));
